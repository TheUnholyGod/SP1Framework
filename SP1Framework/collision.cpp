...
bool colDetection(int levelnumber)
{
	...
	{
			if (map[g_sChar.m_cLocation.Y - 2][g_sChar.m_cLocation.X] == (char)178)
				return false;
	}
	if (g_abKeyPressed[K_LEFT] && g_sChar.m_cLocation.Y > 0)
	{
			if (map[g_sChar.m_cLocation.Y - 1][g_sChar.m_cLocation.X - 1] == (char)178)
				return false;
	}
	if (g_abKeyPressed[K_DOWN] && g_sChar.m_cLocation.X < g_Console.getConsoleSize().X - 1)
	{
			if (map[g_sChar.m_cLocation.Y][g_sChar.m_cLocation.X] == (char)178)
				return false;
	}
...
bool creativeColDetection(int levelnumber)
{
	...
	{
		if (map[g_sCreaChar.m_cCreativeLocation.Y - 2][g_sCreaChar.m_cCreativeLocation.X] == (char)178)
			return false;
	}
	if (g_abKeyPressed[K_LEFT] && g_sCreaChar.m_cCreativeLocation.Y > 0)
	{
		if (map[g_sCreaChar.m_cCreativeLocation.Y - 1][g_sCreaChar.m_cCreativeLocation.X - 1] == (char)178)
			return false;
	}
	if (g_abKeyPressed[K_DOWN] && g_sCreaChar.m_cCreativeLocation.X < g_Console.getConsoleSize().X - 1)
	{
		if (map[g_sCreaChar.m_cCreativeLocation.Y][g_sCreaChar.m_cCreativeLocation.X] == (char)178)
			return false;
	}
